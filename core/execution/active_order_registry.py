...
接口契约：
- register_order(order_id, symbol, side, price, qty, order_type) -> Dict[str, Any] : 注册新订单，若冲突则拒绝
- update_order_qty(order_id, new_qty) -> Dict[str, Any] : 更新指定订单的剩余数量（仅更新数量，不更新价格）
- unregister_order(order_id) -> Dict[str, Any] : 撤销指定订单
- get_active_orders(symbol, side) -> Dict[str, Any] : 查询活跃订单列表，可按品种、方向过滤
- reserve_slot(symbol, side, price, qty, timeout_ms) -> Dict[str, Any] : 原子化预留订单槽位，返回预留令牌
- release_slot(token) -> Dict[str, Any] : 释放预留槽位
- detect_self_trade(symbol, side, price) -> Dict[str, Any] : 检查是否与现有活跃订单构成自成交
...
- 预留槽位具有超时自动释放机制（基于堆的惰性清理），时间堆和预留堆均支持周期性压缩以防止惰性内存泄漏
- 映射表更新均在品种锁外执行，避免死锁
"""
...
class ActiveOrderRegistry:
    ...
    RESERVATION_HEAP_COMPRESS_RATIO = 3.0     # 预留堆压缩触发比率，无量纲，[2.0, 5.0]
    ...
    def _cleanup_expired_reservations(self, symbol: str) -> None:
        ...
        while heap and heap[0][0] < now:
            ...
            if token in reservations:
                ...
                # 幽灵记录：已在 release_slot 中删除，但堆中仍有残留
                ghost_removed += 1
        if removed or ghost_removed:
            logger.debug(
                "预留槽位清理: symbol=%s 移除 %d 个过期槽位, %d 个幽灵记录",
                symbol, removed, ghost_removed
            )
        # 若堆中幽灵记录过多，触发压缩
        if heap and len(heap) > len(reservations) * self.RESERVATION_HEAP_COMPRESS_RATIO:
            self._compress_reservation_heap(symbol)

    def _evict_oldest_if_needed(self, symbol: str, side: str) -> Optional[str]:
        """当超过单品种单方向最大订单数时，淘汰最早订单并返回其ID；若失败返回 None"""
        key = self._get_side_key(symbol, side)
        orders = self._orders_by_symbol.get(symbol, {})
    ...
    def _verify_index_consistency(self, symbol: str, timeout_deadline: Optional[float] = None) -> Tuple[bool, List[str], bool]:
        """
        验证品种级别的索引一致性，返回 (是否一致, 错误列表, 是否超时)
        若超时则仅返回已检查的部分结果
        """
        errors = []
        timed_out = False
        ...
        for side in ("buy", "sell"):
            ...
            if heap and len(heap) > len(actual_ids) * self.HEAP_COMPRESS_RATIO:
                self._compress_time_heap(symbol, side)
        return len(errors) == 0, errors, timed_out
    ...
    def release_slot(self, token: str) -> Dict[str, Any]:
        ...
        with lock:
            ...
            # 堆中惰性删除，由 _cleanup_expired_reservations 负责清理
        ...
        try:
            ...
            for sym in symbols:
                if time.time() - start_time > timeout_sec:
                    all_errors.append(f"健康检查超时: 已检查{checked}/{len(symbols)}品种，剩余未检查")
                    break
                lock = self._get_lock(sym)
                with lock:
                    total_orders += len(self._orders_by_symbol[sym])
                    total_reservations += len(self._reservations_by_symbol.get(sym, {}))
                    consistent, errors, timed_out = self._verify_index_consistency(sym, start_time + timeout_sec)
                    if not consistent:
                        all_consistent = False
                        all_errors.extend(errors)
                    if timed_out:
                        all_errors.append(f"品种 {sym} 索引检查超时")
                        break
                    checked += 1
            ...
            if time.time() - start_time <= timeout_sec:
                with self._mapping_lock:
                    items_to_check = min(1000, len(self._order_id_to_symbol))
                    for oid, sym in list(self._order_id_to_symbol.items())[:items_to_check]:
    ...
    def stress_health_check(self) -> Dict[str, Any]:
        """压力测试自检：模拟高并发注册/撤销，测量P99延迟（仅限非交易时段隔离环境执行）"""
        import random
        results = {}
        warnings = []
